...
export const useOrdersQuery = (options: Partial<OrdersQueryOptions>) => {
  ...
  const { data, isPending, error, refetch, isFetching } = useQuery({
    queryKey: [SELLER_API_ENDPOINTS.POS_ORDER_LIST],
...
export const useOrdersQueryById = (id: string, store_id?: string) => {
  const { find } = useOrdersQueryService();
  const errorToastRef = useRef<string | null>(null);
  const { data, isPending, error, refetch, isFetching } = useQuery({
    queryKey: [SELLER_API_ENDPOINTS.POS_ORDER_DETAILS, id, store_id],
    queryFn: () => find(id, store_id),
    retry: false,
    refetchOnWindowFocus: false,
    enabled: !!id && !!store_id,
  });
