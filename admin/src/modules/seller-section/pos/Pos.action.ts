...
export const useProductListQuery = (options: Partial<ProductQueryOptions>) => {
  ...
  const { data, isPending, error, refetch, isFetching } = useQuery({
    queryKey: [SELLER_API_ENDPOINTS.POS_PRODUCT_LIST, options],
...
export const useProductDetailsQueryBySKU = (
  id: string,
  store_id?: string,
  locale?: string
) => {
  ...
  const { data, isPending, error, refetch, isFetching } = useQuery({
    queryKey: [SELLER_API_ENDPOINTS.POS_PRODUCT_DETAILS, id, store_id],
    queryFn: () => findPageBySlug(id, { language: locale, store_id: store_id }),
    refetchOnWindowFocus: false,
    enabled: !!id && !!store_id,
    retry: false,
  });
...
export const usePosCustomerList = (options: Partial<ProductQueryOptions>) => {
  ...
  const { data, isPending, error, refetch, isFetching } = useQuery({
    queryKey: [SELLER_API_ENDPOINTS.POS_CUSTOMER_LIST, options],
    queryFn: () => findAll(options),
    refetchOnWindowFocus: false,
    ...options,
  });
...
export const usePosCustomerStoreMutation = () => {
  ...
  return useMutation({
    ...
    mutationKey: [SELLER_API_ENDPOINTS.POS_CUSTOMER_STORE],
...
export const useCouponMutation = () => {
  ...
  return useMutation({
    mutationFn: (values: { coupon_code: string | null; sub_total: string }) =>
      create(values),
    mutationKey: [SELLER_API_ENDPOINTS.COUPON_CHECK],
    onSuccess: async (data) => {
...
export const usePlaceOrderMutation = () => {
  ...
  return useMutation({
    ...
    mutationKey: [SELLER_API_ENDPOINTS.POS_PLACE_ORDER],
    onSuccess: async (data) => {
...
export const usePosInvoiceMutation = () => {
  ...
  return useMutation({
    mutationFn: (values: any) => create(values),
    mutationKey: [SELLER_API_ENDPOINTS.POS_INVOICE],
    onSuccess: async (data) => {
...
export const usePosCategoriesQuery = (
  ...
  const { data, isPending, error, refetch, isFetching } = useQuery({
    queryKey: [SELLER_API_ENDPOINTS.POS_CATEGORY, options?.type],
    queryFn: () => findAll(options),
    ...options,
  });
  return {
