...
export const useChatSettingsQuery = (options: Partial<ChatSettingsQueryOptions>) => {
  ...
  const { data, isPending, error, refetch, isFetching } = useQuery({
    queryKey: [SELLER_API_ENDPOINTS.CHAT_SETTINGS],
...
export const useChatSettingsStoreMutation = () => {
  ...
  return useMutation({
    ...
    mutationKey: [SELLER_API_ENDPOINTS.CHAT_SETTINGS],
...
export const useLiveChatQuery = (
  ...
  const { data, isPending, error, refetch, isFetching } = useQuery({
    queryKey: [SELLER_API_ENDPOINTS.LIVE_CHAT_LIST],
    queryFn: () => findAll(options),
    retry: false,
...
export const useChatDetailsQueryById = (options: Partial<LiveChatQueryOptions>) => {
  ...
  const { data, isPending, error, refetch, isFetching } = useQuery({
    queryKey: [SELLER_API_ENDPOINTS.CHAT_DETAILS, options?.receiver_id, options?.receiver_type,],
    queryFn: () => findAll(options),
    refetchOnWindowFocus: false,
...
export const useReplyMessageMutation = () => {
  ...
  return useMutation({
    ...
    mutationKey: [SELLER_API_ENDPOINTS.REPLY_MESSAGE],
    onSuccess: async (data) => {
      if (Boolean(data?.data)) {
      } else {
        toast.error(data?.data?.message);
