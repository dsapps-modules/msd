...
const EditDynamicField = ({ ID }: any) => {
  const t = useTranslations();
  const isRefetch = useAppSelector((state) => state.refetchValue.isRefetch);
  const { DynamicFieldByID, refetch, isPending } = useDynamicFieldQueryById(ID);
  ...
  return (
    <>
      ...
      {isPending || !DynamicFieldByID ? (
        <CardSkletonLoader />
      ) : (
        <Card className="mt-4 ">
          <CardContent className="p-2 md:p-4">
            <CreateOrUpdateDynamicFieldForm data={DynamicFieldByID} />
          </CardContent>
        </Card>
      )}
    </>
