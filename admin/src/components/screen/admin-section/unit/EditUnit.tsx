...
import { useUnitQueryById } from "@/modules/admin-section/unit/unit.action";
...
const EditUnit = ({ ID }: any) => {
  const t = useTranslations();
  const isRefetch = useAppSelector((state) => state.refetchValue.isRefetch);
  const { unit, refetch, isPending } = useUnitQueryById(ID);
      ...
      {isPending || !unit ? (
        ...
        <Card className="mt-4">
          <CardContent className="p-2 md:p-4">
            <CreateOrUpdateUnitForm data={unit} />
          </CardContent>
        </Card>
      )}
    </>
