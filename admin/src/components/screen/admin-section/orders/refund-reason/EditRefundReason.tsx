...
import { useRefundReasonQueryById } from "@/modules/admin-section/orders/refund-reason/refund-reason.action";
...
const EditRefundReason = ({ ID }: any) => {
  const t = useTranslations();
  const pathname = usePathname();
  const isRefetch = useAppSelector((state) => state.refetchValue.isRefetch);
  const { RefundReason, refetch, isPending } = useRefundReasonQueryById(ID);
  ...
  return (
    <div className="space-y-4">
      ...
      {isPending || !RefundReason ? (
        <CardSkletonLoader />
      ) : (
        <>
          <CreateOrUpdateRefundReasonForm data={RefundReason} ID={ID} />
        </>
      )}
    </div>
  );
...
export default EditRefundReason;
