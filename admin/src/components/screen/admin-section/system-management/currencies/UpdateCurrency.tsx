...
import { BaggageClaim, Currency } from "lucide-react";
...
import { useCurrencyQueryById } from "@/modules/admin-section/system-management/currency/currency.action";
import CardSkletonLoader from "@/components/molecules/CardSkletonLoader";

const UpdateCurrency = ({ ID }: any) => {
  const t = useTranslations();
  const isRefetch = useAppSelector((state) => state.refetchValue.isRefetch);
  const { CurrencyByID, refetch, isPending } = useCurrencyQueryById(ID);
  ...
  return (
    <>
      ...
      {isPending || !CurrencyByID ? (
        <CardSkletonLoader />
      ) : (
        <Card className="mt-4 ">
          <CardContent className="p-2 md:p-4">
            <CreateOrUpdateCurrencyForm data={CurrencyByID} />
          </CardContent>
        </Card>
      )}
    </>
