...
} from "@/modules/seller-section/chat/chat.action";
import { useFileUploadService } from "@/modules/seller-section/chat/chat.service";
...
const Chat: React.FC<any> = ({
  ...
  useEffect(() => {
    const stored = localStorage.getItem("chat_target_seller");
    ...
    try {
      ...
      localStorage.removeItem("chat_target_seller");
    } catch (error) {
      toast.error(
  ...
  useEffect(() => {
    ...
    const channel = pusher.subscribe(`activeChannel_${currentUser?.user_id}`);
  ...
  }, [activeChannel, currentUser?.user_id]);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      //After successful DB operation, send message via Pusher
    ...
    <div className="flex h-[calc(100vh-10rem)] bg-gray-100 ">
      ...
      >
        <Card className="p-4 rounded-none">
          ...
          <div className="mt-2 flex flex-col xl:flex-row items-center gap-2 w-full">
            ...
            {originalData.map((user: any, index: any) => (
              <div
                ...
                onClick={() => {
                  ...
                  const currentSenderId = String(store_id);

                  setActiveChannel(clickedUserId);
        ...
        >
          ...
          >
            ...
            {messages.map((msg, index) => {
              const isSender = msg.sender_type !== "store";
          ...
          <Card className="min-h-[120px] p-4 relative rounded-none">
            <div className="p-2 w-auto">
              <span className="font-semibold"></span>{" "}
              <span className="px-2">{selectedFile && selectedFile.name}</span>{" "}
              {selectedFile && (
                <button
                  type="button"
            ...
            <div className="flex flex-col md:flex-row px-4 rounded-md items-center gap-2">
              ...
              <label className="cursor-pointer bg-transparent  px-2 py-2  text-sm text-gray-500 dark:text-white hidden md:block">
                <Paperclip />
                <input
                  type="file"
