...
const DepartmentTable = ({ searchValue }: any) => {
  ...
  const { Departments, refetch, isPending, error } = useDepartmentAllQuery({
  ...
  let totalDataLength = (Departments as any)?.meta?.total;
  const startIndex = (Departments as any)?.meta?.from;
  const LastPage = (Departments as any)?.meta?.last_page;
  const originalData = useMemo(() => {
    const data = (Departments as any)?.data || [];
    return data.map((item: any, index: number) => ({
      ...item,
      serial: startIndex + index,
    }));
  }, [Departments, startIndex]);
  ...
  const { mutate: tagDelete } = useDepartmentDeleteMutation();
  ...
  const useColumn = (
    ...
    const handleEdit = (e: React.MouseEvent, id: string) => {
      const url = `${Routes.editDepartment}/${id}`;
          ...
          render: (o: any, row: RecordType) => (
            <div className="flex items-center gap-2 ">
              ...
              <ConfirmationModal
                ...
                loading={loading}
                title={t("title.delete_department")}
                subTitle={t("sub_title.delete_department")}
              />
            </div>
          ),
        };
