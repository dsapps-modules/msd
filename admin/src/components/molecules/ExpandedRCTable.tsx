...
type Props = {
  ...
  maxWidth?: any;
  selectedRows?: any;
  handleRowSelection?: (id: any) => void;
};
...
const ExpandedRCTable = ({
  ...
  const expandable = {
        ...
        <div className="flex items-center">
          ...
          {hasChildren ? (
            <button
              onClick={(e) => {
                e.stopPropagation();
                setExpandedRowKey(expanded ? null : record.id);
                onExpand(record, e);
              }}
            ...
            <span className="indicator-spacer" />
          )}
        </div>
      );
    },
    ...
    expandedRowRender: (record: RecordType) => {
      return (
        <div className="ml-[24px] bg-[#f9f9f9] border-l-2 border-[#e0e0e0]">
          <Table
            data={record.children || []}
            columns={filteredColumns.map((col: any) => ({
              ...col,
              render: (text: any, childRecord: RecordType) => {
                return (
                  <div
                  ...
                  >
                    {col.render ? col.render(text, childRecord) : text}
                  </div>
                );
              },
            }))}
            ...
            rowClassName={(record) => {
              if (record.id === expandedRowKey) return "!text-blue-600"; // parent
              if (record.menu_level && record.menu_level > 0)
                return "!text-blue-400"; // child
              return "";
            }}
          />
        </div>
