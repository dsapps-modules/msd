...
export async function POST(
  ...
  try {
    ...
    const apiResponse = await fetch(
      `${process.env.NEXT_PUBLIC_REST_API_ENDPOINT}/v1/fornecedor/login`,
      {
        method: "POST",
        headers: {
    ...
    const loginUrl = new URL(`/${locale}/fornecedor/login`, request.url);
    ...
    const response = NextResponse.redirect(
      new URL(`/${locale}/fornecedor/dashboard`, request.url),
      303
    );
    response.cookies.set(AUTH_TOKEN_KEY, data.token, cookies);
    response.cookies.set(AUTH_USER, "fornecedor_level", cookies);
    response.cookies.set(AUTH_CRED, authCred, cookies);

    return response;
  } catch {
    const fallbackUrl = new URL(`/${routeLocale}/fornecedor/login`, request.url);
