...
class InstallController
{
    ...
    {
        // Redirect if app already installed
    ...
    public function permissions()
    {
        ...
        //$basePath = $this->getBasePath(); // Root of Laravel app
        $basePath = $this->getDocumentRoot();
    ...
    public function environment()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $envUpdates = [
                ...
                'APP_KEY'       => 'base64:' . base64_encode(random_bytes(32)),  // 🔥 auto-generate key
            ...
            $targetPath = $this->getDocumentRoot() . DIRECTORY_SEPARATOR . '.env';
            ...
            $projectRoot = $this->getDocumentRoot(); // or your actual Laravel root
            ...
            // Create a writable .composer directory if it doesn't exist
            $composerHome = $projectRoot . '/.composer';
            if (!is_dir($composerHome)) {
                mkdir($composerHome, 0755, true);
            }

            // Set both COMPOSER_HOME and HOME
            ...
            exec('composer install --no-interaction --prefer-dist 2>&1', $outputComposer, $codeComposer);

            if ($codeComposer !== 0) {
                $this->logError("Composer install failed:\n" . implode("\n", $outputComposer));
                header('Location: ?step=environment&error=composer');
                exit;
            }            
            ...
            if (!is_link($publicStorage)) {
                exec('php artisan storage:link 2>&1', $outputStorageLink, $codeStorageLink);
                if ($codeStorageLink !== 0) {
                    $this->logError("Storage link creation failed:\n" . implode("\n", $outputStorageLink));
                    header('Location: ?step=environment&error=storagelink');
                    exit;
                }
            }
            ...
            // Import SQL
            $sqlFile = realpath(__DIR__ . '/../database/fresh_db.sql');
            ...
            if ($installation_mode === 'demo') {
                ...
                if ($sqlFile === false) {
                    ...
                    echo "<pre>Checked path: " . __DIR__ . '/../database/fresh_db.sql' . "</pre>";
                    exit;
                }
                if (file_exists($sqlFile)) {
                    $sql = file_get_contents($sqlFile);
    ...
    public function admin()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $stmt = $pdo->prepare("INSERT INTO users (id, first_name, last_name, slug, phone, email, password, activity_scope,email_verified,status, image)
                       VALUES (?, ?, ?, ?, ?, ?, ?, ?,?,?,?)");
            $stmt->execute([8, $first_name, $last_name, $slug, $phone, $email, $password, 'system_level', 1, 1,'1300']);
    ...
    public function finish()
    {
        ...
        $envPath = $this->getDocumentRoot() . DIRECTORY_SEPARATOR . '.env';
    ...
    private function deleteInstallFolder($dir)
    {
        if (!file_exists($dir)) {
            return true;
        }
        ...
        foreach ($items as $item) {
            ...
            is_dir($path) ? $this->deleteInstallFolder($path) : @unlink($path);
        }
    ...
    // Better approach - Create a helper method for base path
    private function getBasePath()
    {
        ...
        if ($basePath === null) {
            // From: public/install/controllers/InstallController.php
            // To:   project-root/
